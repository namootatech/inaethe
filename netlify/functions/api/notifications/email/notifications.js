...
const sendSubscriptionsWelcomeEmail = async (mongoClient, data) => {
    ...
    const partnerCollection = mongoClient.db(process.env.SELECTED_DB).collection("partners");
    ...
    const from = {
        email: "subcriptions@helpem.co.za",
        name: "Helpem Subscriptions"
    }
    const subject = "Welcome to Helpem";
    ...
    const personalization = [
        {
          ...
          data: {
            ...
            partner: {
              ...
              cause: partner.cause,
              goals: partner.goals
              ? partner.goals.slice(0, -1).join(', ') + (partner.goals.length > 1 ? ' & ' : '') + partner.goals.slice(-1)
              : "making the world a better place",
            }
          },
        },
      ]
    return emails.sendTemplateEmail({to, from, subject, templateId, personalization})
}

  ...
  switch (status?.toLowerCase()) {
    case 'pending':
      return (
        <Badge
          variant='outline'
          className='bg-yellow-900/20 text-yellow-500 border-yellow-500/50'
        >
          Pending
        </Badge>
      );
    case 'approved':
      return (
        <Badge
          variant='outline'
          className='bg-green-900/20 text-green-500 border-green-500/50'
        >
          Approved
        </Badge>
      );
    case 'completed':
      return (
        <Badge
          variant='outline'
          className='bg-blue-900/20 text-blue-500 border-blue-500/50'
        >
          Completed
        </Badge>
      );
    case 'rejected':
      return (
        <Badge
          variant='outline'
          className='bg-red-900/20 text-red-500 border-red-500/50'
        >
          Rejected
        </Badge>
      );
    default:
      return (
        <Badge
          variant='outline'
          className='bg-gray-900/20 text-gray-500 border-gray-500/50'
        >
          Processing
        </Badge>
      );
...
export default function Earnings() {
  ...
  const fetchTransactions = useCallback(
    async (page = 1) => {
      if (!user?.user?._id || !api || !isMounted.current) return;

      try {
        const response = await api.getUserNetworkTransactions(
          user.user._id,
          page,
          ITEMS_PER_PAGE,
    ...
    [api, user, transactionDateFilter]
  );
  ...
  const fetchWithdrawals = useCallback(
    async (page = 1) => {
      ...
      try {
        const response = await api.getUserWithdrawalRequests(
          user.user._id,
          page,
          ITEMS_PER_PAGE,
    ...
    [api, user, withdrawalStatusFilter]
  );
  ...
  const fetchEarnings = useCallback(async () => {
    ...
    try {
      const response = await api.getUserEarnings(user.user._id);
  ...
  }, [user, api, fetchTransactions, fetchWithdrawals, fetchEarnings]);

  // Handle transaction date filter change - only refetch if the component is mounted and not in loading state
  useEffect(() => {
    if (!loading && initialTransactionsFetchDone.current && isMounted.current) {
  ...
  useEffect(() => {
    if (!loading && initialWithdrawalsFetchDone.current && isMounted.current) {
  ...
  return (
    <div className='space-y-6'>
      <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-4'>
        <h1 className='text-3xl font-bold text-gray-100'>Earnings Dashboard</h1>
          ...
          <DialogContent className='bg-gray-800 border-gray-700 text-gray-100'>
            <DialogHeader>
              ...
              <DialogDescription className='text-gray-400'>
                Enter the amount you would like to withdraw from your available
                balance.
              </DialogDescription>
            </DialogHeader>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
        <Card className='bg-gray-800 border-gray-700 overflow-hidden'>
          <CardHeader className='pb-2'>
            <CardTitle className='text-gray-300 flex items-center text-sm font-medium'>
          ...
          <CardContent>
            ...
            <p className='text-xs text-gray-400'>
              Lifetime earnings from your network
            </p>
          </CardContent>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className='w-full'>
        ...
        <TabsContent value='overview' className='space-y-4 mt-6'>
          <Card className='bg-gray-800 border-gray-700'>
            ...
            <CardContent>
              <div className='h-[300px] flex items-center justify-center bg-gray-700/30 rounded-md border border-gray-700'>
                  ...
                  <div className='space-y-3'>
                    {transactions.slice(0, 3).map((transaction, index) => (
                      <div
                        ...
                        className='flex items-center justify-between p-3 rounded-md bg-gray-700/30 border border-gray-700'
                      >
                        <div className='flex items-center'>
                          <div className='rounded-full bg-green-900/20 p-2 mr-3'>
                ...
                {withdrawalRequests && withdrawalRequests.length > 0 ? (
                  <div className='space-y-3'>
                    {withdrawalRequests.slice(0, 3).map((request, index) => (
                      ...
                      >
                        <div className='flex items-center'>
                          <div className='rounded-full bg-blue-900/20 p-2 mr-3'>
          ...
          <Card className='bg-gray-800 border-gray-700'>
            <CardHeader>
              <div className='flex flex-col md:flex-row md:items-center justify-between gap-4'>
                <div>
                  ...
                  <CardDescription className='text-gray-400'>
                    View all earnings from your network
                  </CardDescription>
                </div>
            ...
            <CardContent>
              {transactions && transactions.length > 0 ? (
                <div className='rounded-md border border-gray-700 overflow-hidden'>
                  <Table>
                    <TableHeader className='bg-gray-700/50'>
                      <TableRow className='hover:bg-gray-700/70 border-gray-600'>
                      ...
                      {transactions.map((transaction, index) => (
                        ...
                        >
                          ...
                          <TableCell className='text-right text-gray-300'>
                            R{Number.parseFloat(transaction.amount).toFixed(2)}
                          </TableCell>
                ...
                <div className='text-center py-12 border border-gray-700 rounded-md'>
                  <div className='rounded-full bg-gray-700/50 p-3 inline-flex mb-4'>
              ...
              {transactionsTotalPages > 1 && (
                <div className='mt-4'>
                  <Pagination>
                    <PaginationContent>
                      <PaginationItem>
                        <PaginationPrevious
                          onClick={() =>
                            fetchTransactions(Math.max(1, transactionsPage - 1))
                          }
                          className={
                            transactionsPage === 1
                  ...
                  <div className='text-center mt-2 text-xs text-gray-400'>
                    Showing page {transactionsPage} of {transactionsTotalPages}{' '}
